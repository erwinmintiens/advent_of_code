...
def read_card_from_line(line: str) -> Card:
    card_id = int(line.split(":")[0].strip().split()[1].strip())
    winning_numbers = [
        number.strip()
        for number in line.split("|")[0].strip().split(":")[1].strip().split()
    ]
    your_numbers = [number.strip() for number in line.split("|")[1].strip().split()]
