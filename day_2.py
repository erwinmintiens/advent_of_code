...
class Game:
    def __init__(self, id, red=0, green=0, blue=0) -> None:
        self.id = id
        self.red = red
        self.green = green
        self.blue = blue

    def check_if_possible(self, max_red, max_green, max_blue) -> bool:
        return not (
            self.red > max_red or self.green > max_green or self.blue > max_blue
        )
...
def extract_game(line: str) -> tuple[int, list]:
    ...
    results = [result.strip() for result in results_line.split(";")]
...
def extract_colors(game_id: int, results: list):
    ...
    for result in results:
        ...
        for color in colors:
            if "green" in color:
                value = int(color.replace(" green", ""))
                if value > game.green:
                    game.green = value
            elif "red" in color:
                value = int(color.replace(" red", ""))
                if value > game.red:
                    game.red = value
            elif "blue" in color:
                value = int(color.replace(" blue", ""))
                if value > game.blue:
                    game.blue = value
    return game.power()
